...
const ARView: React.FC = () => {
  ...
  const [selectedLandform, setSelectedLandform] = useState(0);
  ...
  const landforms: Landform[] = [
    {
      ...
      description: '洼地地形',
      elevation: '1,240m',
      imageUrl: 'https://lh3.googleusercontent.com/aida-public/AB6AXuAoXAmVCVEIgASFzpupajADQJAb5143WKjF0k11GpLHN71xB2jDOgA07ag2Ak0o4rL1W2pHwUEKa3cPpL0gdIk54B3g5bahw4SjPxYPZm2kVfnz8WBBtWmxiR_y7ALNTUib-nPXMeTJm5xq3xNKRLjdXh_JghebM4Nls_3fxzpAridnlCj7D7-PSC7JzjF2TOHkCnK_gF4jPYr7jxKzEV-VVDoMYbXPV1e5jfG8M_q5gRVU82kHlfNcJEMJg_6Ro_Z8fH0MjrPuI4TV',
    },
    {
      id: 'peak',
      name: '山峰',
    ...
    },
    {
      ...
      description: '垂直落差',
      elevation: '900m',
      imageUrl: 'https://lh3.googleusercontent.com/aida-public/AB6AXuBPhbbndKJv9XTmDhnfCUixfjC9B5RJ7FlGj3mPluThV6pBadflks3DXszXQ4ewuvGKzvwQFkUfvLmhl7bTbawbAZ4zWUpvBVipjIX-zqlVEErd1x9mZawxXnuK-BYdDA5t3AXDY4afolw6S_6ktlHNjzR__h9LZiMdE50rPIgfPj5OFRWvLdSeMWQJ6EM9kvSyAHUHq_tGKANQPyHDfF5lWWgu1Oa7xwqXfQdu7DgvgeK3_bv50DgfCHON90oGLihJJJfEQmrpSjnJ',
    },
  ];

  const currentLandform = landforms[selectedLandform];
    ...
    <div className="bg-background-dark text-white font-display h-screen w-full overflow-hidden relative select-none">
        ...
        <div className="glass-panel rounded-full p-2 pl-4 pr-2 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-primary-cyan leading-none">地形: {currentLandform.name}</p>
            </div>
          </div>
      ...
      <footer className="absolute bottom-0 left-0 right-0 z-50 flex flex-col justify-end bg-gradient-to-t from-background-dark via-background-dark/90 to-transparent pt-12 pb-6 px-4">
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 snap-x">
          {landforms.map((landform, index) => (
            <div 
              key={landform.id}
              onClick={() => setSelectedLandform(index)}
              className={`snap-center shrink-0 w-40 cursor-pointer hover:scale-[1.02] transition-transform ${
                selectedLandform === index ? 'w-48' : ''
              }`}
            >
              <div className={`h-28 rounded-xl ${
                selectedLandform === index 
                  ? 'bg-gradient-to-r from-primary-cyan to-[#92c0c9] p-[2px]' 
                  : 'glass-panel'
              }`}>
                <div className={`relative h-full rounded-xl ${
                  selectedLandform === index ? 'bg-[#192f33]' : ''
                } p-3 flex flex-col justify-end border border-white/5 overflow-hidden`}>
                  <div 
                    className="absolute inset-0 bg-cover bg-center opacity-30" 
                    style={{ backgroundImage: `url("${landform.imageUrl}")` }}
                  ></div>
                  {selectedLandform === index && (
                  ...
                  <div className="relative z-10">
                    <p className="text-white text-base font-bold leading-tight">{landform.name}</p>
                    <p className="text-white/50 text-xs">{landform.description}</p>
                  </div>
                </div>
              </div>
            </div>
